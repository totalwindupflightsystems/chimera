...
2026-09-25 (run 16) | SHIPPABLE | promise: "custom OpenAI-compatible endpoints — any provider Chimera does not route natively is called through its own base_url over the OpenAI SDK" → HELD across all 4 entry points (8th consecutive SHIPPABLE; the custom-provider seam was the one runs 1-15 never touched) | t2fs: ~4 min (wheel install 43s + minimal config + first custom-provider answer 14.6s; live-REST path 10.9s) | friction 4 (incl. one P1 trust defect) | findings: shared ~/.chimera/blocked-models.json persists fingerprintless credential blocks 7 days, never self-clear, contradicts CLI remedy text (DF-CHIMERA-V2-58 P1, repro'd with wheel registry, escape verified: delete file); smoke_live.py exits 0 on STALE + parity is checkout-relative → CURRENT lie on foreman-moved workdir (DF-CHIMERA-V2-60 P1); workers-vs-stages formation typo silently becomes auto preset (DF-CHIMERA-V2-59 P2); catalog entry required for custom-provider dispatch, undocumented (DF-CHIMERA-V2-61 P2); 9router ds/ appends data:[DONE] after non-stream JSON (DF-CHIMERA-V2-63 P3, upstream) | live deploy: STALE vs origin/main 41947e5 (3 user-facing merges 14:07-14:59 not reloaded; smoke PASSed it anyway) | install: SKIPPED-install-bunker — las-03 offline 3h+ (ssh timeout x3), las-02 bunkerd stuck activating 30+ min, bunker list refused (DF-CHIMERA-V2-62; 8th skip, 12 runs proved install) | smoke=ok (live REST battery + scratch-instance runs all real, 0 worker failures) | no PERF row (warm 25.6-31.1s model-bound, cold 14.6s, install 43s — nothing user-felt) | artifacts: docs/dogfood/2026-09-25-run16-integration.md, diagnostics.md run-16 leg, usage skill v1.8.0, board rows DF-CHIMERA-V2-58..63 | lesson: control-probe upstreams with raw curl BEFORE blaming the seam — turned two jq parse errors into two named 9router defects; shared-checkout discipline: artifacts committed with explicit paths after HEAD re-verification (foreman judge was swinging branches mid-run)
